...
export const CreateUser = (req: e.Request, res: e.Response) => {
	...
	if (!e || !p) return res.status(400).json({ error: "Email and/or Password field is empty!" });

	auth
		.createUserWithEmailAndPassword(e, p)
		.then(async (userData: firebase.auth.UserCredential) => {
...
export const SignIn = (req: e.Request, res: e.Response) => {
	...
	if (!e || !p) return res.status(400).json({ err: "Email and/or Password field is empty!" });
